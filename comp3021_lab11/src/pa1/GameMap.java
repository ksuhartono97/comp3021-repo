...
public class GameMap 
{
	...
	public static int TERRAIN_EMPTY = 3;
	public static int TERRAIN_BLOCKED = 0;
	public static int TERRAIN_OCCUPIED = 4;
	public static int TERRAIN_SOIL = 1;
	public static int TERRAIN_SAND = 2;
	
	public static char TERRAIN_EMPTY_CHAR = ' ';
	public static char TERRAIN_RIVER_CHAR = '0';
	public static char TERRAIN_SOIL_CHAR = '1';
	public static char TERRAIN_SAND_CHAR = '2';
	...
	public void loadTerrainMap(File filename) throws IOException
	{
		...
		try (Scanner reader = new Scanner(inputFile)){
			...
			for(int y = 0; y < height; y++) {
				...
				for (int x = 0; x < width; x++) {
					if(lineArr[x] != '0' && lineArr[x] != '1' && lineArr[x] != '2') {
						...
						switch (lineArr[x]) {
							case '0':
								...
								break;
							case '1':
								this.terrainMap[y][x] = TERRAIN_SAND;
								break;
							case '2':
								this.terrainMap[y][x] = TERRAIN_SOIL;
								break;
						}
					}
				}
	...
	public void renderMap(ArrayList<Player> players)
	{
		...
		for (int y = 0; y < height; ++y) {
			for (int x = 0; x < width; ++x) {
				...
				switch (terrainMap[y][x]) {
					case 0:
						mapCharacter = TERRAIN_RIVER_CHAR;
						break;
					case 1:
						mapCharacter = TERRAIN_SAND_CHAR;
						break;
					default:
						mapCharacter = TERRAIN_SOIL_CHAR;
						break;
				}
		...
		for (Player pl : players ) {
			for (Unit un : pl.getUnitList()) {
				if(un.isAlive() && un.getLocationX() != -1 && un.getLocationY() != -1) {
